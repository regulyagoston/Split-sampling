...
function b_0 = est_multi_NS( N , S , MC , objY , objX , distX , distE , ordered , a_l_Y , a_u_Y )
...
if max( N ) * MC <= crit_num
    ...
    for s = 1 : nS
        ...
        b_0( s , : , : ) = estimate_DOC( Y , X , N , S( s ) , objY , objX , ordered );
    end    
else
    ...
    for k = 1 : K 
        ...
        for s = 1 : nS
            disp( [ 'Number of sub-sample: ' , num2str( S( s ) ) ] )
            b_1 = estimate_DOC( Y , X , N , S( s ) , objY , objX , ordered );
            b_k( s , : , : ) = b_1;
        end
